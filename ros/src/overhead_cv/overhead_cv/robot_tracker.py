...
class RobotTracker(Node):
    ...
    def publish_pixels_as_positions(self, points):
        ...
        points = [
            ...
            for (xx, yy), (_, _, z) in zip(
                map(pixel_to_world, [(x, y) for x, y, _ in points]), points
            )
        ]
    ...
    def analyze_contours(
        self, contours, frame_dims, contour_size, frame
    ) -> List[Tuple[int, int]]:
        """
    ...
    def image_callback(self, data):
        ...
        if detected_contours:
            for robot_contour in detected_contours:
                ...
                if packet_point:
                    ...
                    # Create a frame that masks around each robot contour to
                    # only find the related blue point
                    frame_angle = np.ones_like(frame, dtype=np.uint8) * 255
                    frame_angle[y_start:y_end, x_start:x_end] = frame[
                    ...
                    angle_points, angle_mask = detect_and_draw_boxes(
                        frame_angle, angle_lower_color, angle_upper_color
                    )
                    ...
                    packet_points.append(
                        [packet_point[0][0], packet_point[0][1], angle]
                    )
