...
class MCTSNode:
    ...
    def best_child(self, c_param=1.414, heuristic_weight=1.0):
        ...
        for child in self.children:
            ...
            # UCB
            exploration = c_param * math.sqrt((2 * log_n / child.n))
    
            # heuristic
            h_val = child.state.calculate_heuristic()
            heuristic_term = (heuristic_weight * h_val) / (child.n + 1)
            
            choices_weights.append(exploitation + exploration + heuristic_term)
    ...
    def find_solution(self, simulations=2000):
